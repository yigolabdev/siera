  ...
  return (
    <nav className={navClasses}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        {/* Logo */}
        <a href="#" className="text-xl tracking-[0.2em] font-bold uppercase z-50">
          SIERRA
        </a>
        ...
        <div className="hidden md:flex space-x-8 text-sm font-medium tracking-wide">
          ...
          <a href="#membership" className="hover:opacity-70 transition-opacity">멤버십</a>
          <a href="#faq" className="hover:opacity-70 transition-opacity">FAQ</a>
        </div>

        {/* Desktop CTA */}
        <div className="hidden md:block">
           <a href="#apply" className={`px-6 py-2 text-xs font-bold uppercase tracking-wider border transition-all ${isScrolled ? 'border-slate-900 hover:bg-slate-900 hover:text-white' : 'border-white hover:bg-white hover:text-slate-900'}`}>
            상담 신청
          </a>
        </div>
      ...
      <div className={`fixed inset-0 bg-slate-900 text-white flex flex-col items-center justify-center space-y-8 transition-transform duration-500 ease-in-out md:hidden ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <a href="#membership" onClick={() => setIsMobileMenuOpen(false)} className="text-2xl font-light">멤버십</a>
        <a href="#faq" onClick={() => setIsMobileMenuOpen(false)} className="text-2xl font-light">FAQ</a>
        <a href="#apply" onClick={() => setIsMobileMenuOpen(false)} className="mt-8 px-8 py-3 bg-white text-slate-900 text-sm font-bold tracking-widest uppercase">
          상담 신청하기
        </a>
      </div>
    </nav>
