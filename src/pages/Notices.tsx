              ...
              <div className="space-y-4">
                {pinnedNotices.map((notice) => (
                  <div key={notice.id} className="card border-l-4 border-red-600 bg-red-50">
                    ...
                    <p className="text-slate-700 text-base leading-relaxed">{notice.content}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              {regularNotices.map((notice) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <h3 className="text-xl font-bold text-slate-900 flex-grow">{notice.title}</h3>
                    <span className="text-sm text-slate-500 whitespace-nowrap ml-4">
                      {notice.date}
                    </span>
                  </div>
          ...
          <div className="sticky top-24">
            <div className="card bg-gradient-to-br from-primary-50 to-green-50">
              ...
              <div className="space-y-4 mb-6">
                {payments.map((payment) => (
                  <div key={payment.id} className="p-4 bg-white rounded-lg border">
                    <div className="flex items-start justify-between mb-2">
                      <h3 className="font-bold text-slate-900 text-lg">{payment.title}</h3>
              ...
              <div className="border-t pt-4">
                <h3 className="font-bold text-slate-900 mb-2 text-lg">계좌 정보</h3>
                <div className="space-y-1 text-slate-700">
